...
class DMSConvert
{
    public static function myConversion($dms)
    {
        if (str_contains($dms,"°")) {


        $arrs = preg_split("/[\s,°\'\"]+/", $dms);
       // dd($arrs);
        $ref = $arrs[0];
        $degrees = $arrs[1];
        $minutes = $arrs[2] / 60;
        $secunds = $arrs[3] / 3600;
        $result = $degrees + $minutes + $secunds;

        if($ref == "Sul" || $ref == "Oeste"){
            return -abs($result);
        }else{
            return $result;
        }
    ...
    public function convert($latlng)
    {
        ...
        //transforma em ingles
        $latlng = str_replace("Sul","S",$latlng);
        $latlng = str_replace("Oeste","W",$latlng);
