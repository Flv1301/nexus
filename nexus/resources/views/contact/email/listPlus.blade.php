            ...
            @foreach($person->emails as $email)
                <tr>
                    ...
                    <td><i class="fa fa-md fa-fw fa-trash text-danger"
                           ...
                           title="Remover"></i></td>
                    <input type="hidden" id="emails_{{ $loop->index }}" name="emails[]" value="{{json_encode($email)}}">
                </tr>
            @endforeach
            </tbody>
        </table>
...
@push('js')
    <script>
        function addEmail() {
            ...
            input.setAttribute('id', 'emails_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9));
