...
@section('title','Log de Acesso Facebook')
@section('plugins.Datatables', true)
@section('plugins.Chartjs', true)
<x-page-header title="Log de Acesso Facebook ({{$file->account_identifier}})"/>
@section('content')
            ...
            <div class="tab-content">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <span class="text-info">{{$file->registration_date}}</span>
                            </div>
                            <div class="form-group">
                                <span>IP do Cadastro: </span>
                                <span class="text-info">{{$file->registration_ip}}</span>
                            </div>
                            <div class="form-group">
                                <span>Número do Telefone: </span>
                                <span class="text-info">{{$file->phone_numbers}}</span>
                                @if($file->phone_numbers_verified)
                                    <span> Verificado em  </span>
                                    <span class="text-info">{{$file->phone_numbers_verified}}</span>
                                @endif
                            </div>
                            <div class="form-group">
                                <span>Primeiro Nome: </span>
                                <span class="text-info">{{$file->first_name}}</span>
                            </div>
                            <div class="form-group">
                                <span>Último Nome: </span>
                                <span class="text-info">{{$file->last_name}}</span>
                            </div>
                            <div class="form-group">
                                <span>Email de Cadastro: </span>
                                <span class="text-info">{{$file->registered_email_addresses}}</span>
                            </div>
                            <div class="form-group">
                                <span>Conta Vinculada: </span>
                                <span class="text-info">{{$file->vanity_name}}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="tab-pane" id="tab_ip_address" role="tabpanel">
                    <x-adminlte-datatable id="tab_ip"
                                          ...
                                          :config="$config"
                                          striped hoverable>
                        @foreach($file->acesslogIpAddress()->leftJoin('ipinfos','facebook_accesslog_ip_address.ip_address','=','ipinfos.ip')->get() as $ip)
                            <tr>
                                <td>{{$ip->ip_address}}</td>
                                <td class="text-sm">{{$ip->time}}</td>
                                <td class="text-sm">{{$ip->name ?? $ip->org ?? ''}}</td>
...
@endsection
@php
    $ips = $file->acesslogIpAddress;
    $ips = $ips->countBy('ip_address')->sortDesc()->take(5);
@endphp
@push('js')
    <script>
        $(function () {
