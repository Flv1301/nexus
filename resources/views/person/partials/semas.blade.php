<div class="card">
    <div class="card-body">
        @if(empty($semas) || count($semas) === 0)
            <div class="alert alert-info">Nenhum registro SEMAS encontrado para {{ $documento }}.</div>
        @else
            @php
                // Coletar todas as chaves presentes nos itens SEMAS (normaliza objects)
                $allKeys = [];
                foreach ($semas as $row) {
                ...
                // Chaves preferenciais (ajustar conforme payload típico)
                $preferred = ['id', 'tipo', 'data', 'descricao', 'municipio'];
                $ordered = [];
            ...
            <div class="d-flex justify-content-between align-items-center mb-2">
                <small class="text-muted">Total registros: {{ count($semas) }}</small>
            </div>
